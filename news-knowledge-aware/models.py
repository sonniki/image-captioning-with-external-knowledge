...
class EntityEncoder(nn.Module):
    ...
    def forward(self, entities, facts, word_embedding):
        """
        Forward propagation.

        :param entities: entities to encode
        :param facts: facts from the knowledge context
        :param word_embedding: the embedding layer for vocabulary words
        ...
        # Count in the article (incl. headline).
        entities_encoded[:, :, 0] = entities[:, :, 1]
        # Presence in the headline.
        entities_encoded[:, :, 1] = entities[:, :, 2]
        # Presence in the first paragraph.
        entities_encoded[:, :, 2] = entities[:, :, 3]
        ...
        entities_encoded[:, :, 4] = counts_indicator
        # Store an embedding of the entity type.
        original_type = entities[:, :, 4].clone().long().to(device)
        entities_encoded[:, :, 5:] = self.type_embedding(original_type)
        entities_encoded = entities_encoded.to(device)
        # Embed the words in the entity name and take average.
        name_idxs = entities[:, :, 5:].clone().long().to(device)
        embedded_name = word_embedding(name_idxs)
        avg_embedded_name = torch.mean(embedded_name, dim=-2)
        #
        entities_encoded = entities_encoded * avg_embedded_name
...
class DecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        type_embedding = nn.Embedding(20, emb_dim - 5)
        self.entity_encoder = EntityEncoder(emb_dim, type_embedding)
        self.num_predicates = 3500
    ...
    def forward(self, captions, encoder_out, caption_masks, caption_lengths, entities, facts):
        ...
        entities_encoded = self.entity_encoder(entities, facts, self.word_embedding) 
